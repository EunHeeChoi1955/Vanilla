...
// 2. 선택된 버튼을 등록후 사용
  ...
  var cnt=0;
    ...
    // cnt = 1;
    // 클래스와 태그이름은 중복가능
    // 그래서  document.querySelector 선택자로 사용하는 경우는
    // 반드시 선택자에 배열 구조로 곽괄호를 사용한다. [0] [1] [2]...

    // 슬라이드
    // 클래스 이름은 ( 한개인경우)querySleector사용
    var slideWrap = document.querySelector('.slide-wrap');

    // 태그이름 여러개(복수)
    var aTag = document.querySelectorAll('.nav li a');

    // 클래스 이름은 (1개인경우)querySleector 사용
    var slideWrap2 = document.querySelector('.slide-wrap');
    ...
    //slideWrap.style = 'left:calc(-1920px*1)'; //클래스라서 뒤에 []배열 넣어주기
    slideWrap.style = 'left:' + (-1920*cnt) + 'px'; //클래스라서 뒤에 []배열 넣어주기


    console.log('slideWrap[0]', slideWrap[0] );
    console.log('slideWrap[0]', slideWrap2[0] );


    console.log('aTag', aTag ); //개체의 배열 내용이 나타나고
    console.log('aTag', aTag[0] );  //개체의 인덱스 값별로 출력
    console.log('aTag', aTag[1] );  //
    console.log('aTag', aTag[2] );
    console.log('aTag', aTag[3] );
